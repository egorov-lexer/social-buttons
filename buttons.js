(function($, w, d, undefined) {
    ...
    var OdklButton = function($context, conf, index) {
        this.init($context, conf, index);
        this.type = 'odnoklassniki';
    };
    OdklButton.prototype = new Button;
    OdklButton.prototype
    = $.extend(OdklButton.prototype,
    {
        /*@methods*/
        countLikes: function() {
            var	serviceURI = this.getCountLink(this.linkToShare) + '&uid=' + this.index;
        ...
        /*@properties*/
        countServiceUrl: 'http://www.odnoklassniki.ru/dk?st.cmd=extOneClickLike&ref='
    });

    // костыль для Odnoklassniki
    w.socialButtonCountObjects = {};

    function Odkl_updateCountOC(e, count, a, d) {
        
        var button = w.socialButtonCountObjects[e];
        if(count > 0) {
            button.setCountValue(count);
        }
        delete w.socialButtonCountObjects[e];
    }

    if(!w.ODKL) {
        w.ODKL = {
            updateCountOC: function(e, count, a, d) {
                Odkl_updateCountOC(e, count, a, d);
            }
        }
    } else {
        var originalOdklUpdate = w.ODKL.updateCountOC;

        w.ODKL.updateCountOC = function(e, count, a, d) {
            Odkl_updateCountOC(e, count, a, d);
            originalOdklUpdate.call(w.ODKL, e, count, a, d);
        };
    }
    ...
    $.fn.socialButton = function(config) {
        this.each(function(index, element) {
            setTimeout(function() {
                ...
                } else if($element.is(conf.selectors.odklButton)) {
                    b = new OdklButton($element, conf, Button.lastIndex);
                }
